...
import { enrichCargoTypeWithImage, enrichCargoTypesWithImages } from '../services/enrichment.service';
...
export const createCargoType = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const enriched = await enrichCargoTypeWithImage(cargoType);
...
export const getAllCargoTypes = async (req: Request, res: Response) => {
  const locale = getLocaleFromRequest(req);
  try {
    const cached = await getCachedCatalog<Record<string, unknown>[]>(CARGO_TYPES_CACHE_KEY);
    const payload =
      cached ??
      (await (async () => {
        const cargoTypes = await CargoType.findAll();
        const rows = cargoTypes.map((item) => item.toJSON() as Record<string, unknown>);
        await setCachedCatalog(CARGO_TYPES_CACHE_KEY, rows);
        return rows;
      })());

    const enriched = await enrichCargoTypesWithImages(payload);
